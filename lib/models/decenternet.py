...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        config_bone = self.spec.HEAD_BONE
        ...
        self.transition_bone = self._make_transition_for_head(
                    inp_channels, config_bone['NUM_CHANNELS'])
        self.transition_locmap = self._make_transition_for_head(
                    inp_channels, config_heatmap['NUM_CHANNELS'])
        self.transition_offset = self._make_transition_for_head(
                    inp_channels, offset_channels)
        
        self.head_heatmap = self._make_heatmap_head(config_heatmap, 2)
        self.head_bone = self._make_bone_head(config_bone)
        self.head_locmap = self._make_heatmap_head(config_heatmap, 1)
    ...
    def _make_heatmap_head(self, layer_config, num_ch):
        heatmap_head_layers = []
        ...
        heatmap_conv = nn.Conv2d(
            ...
            out_channels=self.num_joints*num_ch,
    ...
    def _make_bone_head(self, layer_config):
        bone_head_layers = []
        ...
        bone_head_layers.append(feature_conv)

        bone_conv = nn.Conv2d(
            in_channels=layer_config['NUM_CHANNELS'],
            out_channels=2*layer_config['NUM_BONES'],
            kernel_size=self.spec.FINAL_CONV_KERNEL,
            stride=1,
            padding=1 if self.spec.FINAL_CONV_KERNEL == 3 else 0
        )
        bone_head_layers.append(bone_conv)
        
        return nn.ModuleList(bone_head_layers)
    ...
    def forward(self, x):
        ...
        bone = self.head_bone[1](
        self.head_bone[0](self.transition_bone(x)))
                
        locmap = self.head_locmap[1](
            self.head_locmap[0](self.transition_locmap(x)))
        ...
        return heatmap, offset, bone, locmap
