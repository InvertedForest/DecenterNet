...
class CocoKeypoints(CocoDataset):
    def __init__(self, cfg, dataset, heatmap_generator=None, offset_generator=None, bone_generator=None, transforms=None):
    ...
    def __getitem__(self, idx):
        ...
        bone_map, bone_weight = self.bone_generator(
            joints_list[0], area)
        bone_weight = mask_list[0]*bone_weight

        return img, heatmap, mask, offset, offset_weight, bone_map, bone_weight
    ...
    def get_joints(self, anno):
        ...
        if self.scale_aware_sigma:
            joints = np.zeros((num_people, self.num_joints_with_center, 4))
        else:
