...
def my_get_multi_stage_outputs(cfg, model, image, with_flip=False):
    # forward
    heatmap, offset, _, locmap = model(image)
    if with_flip:
        if 'coco' in cfg.DATASET.DATASET:
            flip_index_single = FLIP_CONFIG['COCO']
            flip_index_double = FLIP_CONFIG['COCO_34_OFFSET']
        elif 'crowd_pose' in cfg.DATASET.DATASET:
            flip_index_single = FLIP_CONFIG['CROWDPOSE_14']
            flip_index_double = FLIP_CONFIG['CROWDPOSE_28']
        else:
            raise ValueError('Please implement flip_index \
                for new dataset: %s'% cfg.DATASET.DATASET)
        ...
        image = torch.flip(image, [3])
        image[:, :, :, :-3] = image[:, :, :, 3:]
        heatmap_flip, offset_flip, _, locmap_flip = model(image)

        heatmap_flip = torch.flip(heatmap_flip, [3])
        locmap_flip = torch.flip(locmap_flip, [3])
        heatmap = (heatmap + heatmap_flip[:, flip_index_double, :, :])/2.0
        locmap = (locmap + locmap_flip[:, flip_index_single, :, :])/2.0

        # offset_flip = offset_to_pose(offset_flip, flip_index=flip_index_double)
        offset_flip = torch.flip(offset_flip, [3])
        offset_flip[:, flip_index_double[::2], :, :] = -offset_flip[:, flip_index_double[::2], :, :]
        offset = (offset + offset_flip[:, flip_index_double, :, :]) / 2.0
        
    return heatmap, offset, locmap
...
def my_aggregate_results(
        cfg, heatmap_sum, heatmap, scale
):
    """
    Get initial pose proposals and aggregate the results of all scale.
